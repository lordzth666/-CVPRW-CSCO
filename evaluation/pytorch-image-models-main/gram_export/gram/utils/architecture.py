...
from gram_export.gram.utils.basic_layers import *
from gram_export.gram.utils.proto_mapper import ProtoMapper
from gram_export.gram.utils.block_manager import channel_block_manager, get_output_channels
...
class Architecture(nn.Module):
    ...
    def configure_drop_path_with_linear(self, drop_path_rate):
        # Configuring drop path blocks with linearly increase rate.
        ...
        for i in range(len(self.layers)):
            ...
            if isinstance(layer, Add):
                total_stages += 1
        if total_stages == 0:
            print("No drop connect needs to be configured!")
            return
        # Now, put stage count and configure dp.
        stage_cnt = 0
        for i in range(len(self.layers)):
            layer = self.layers[i]
            if isinstance(layer, Add):
                stage_cnt += 1
            cur_dp_rate = max_drop_path_rate * float(stage_cnt) / float(total_stages)
            if isinstance(layer, Add):
                print(f"Configuring layer {layer} with drop path rate: {cur_dp_rate}")
                layer.set_drop_connect_rate(cur_dp_rate)
