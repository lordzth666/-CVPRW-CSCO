...
__all__ = ['SwinTransformerCoder']  # model_registry will add each entrypoint fn to this
...
def kaiming_normal_matrix(in_planes, out_planes):
    weight = torch.zeros([out_planes, in_planes], dtype=torch.float)
...
def load_matrix_pair(in_planes, gen_planes, model_dir, inst="normal") -> torch.Tensor:
    if inst == "normal":
        fc1_save_name = os.path.join(model_dir,
            "plain_{}_{}.fc1.npy".format(in_planes, gen_planes))
        fc2_save_name = os.path.join(model_dir,
            "plain_{}_{}.fc2.npy".format(in_planes, gen_planes))
        print("Loading plain_{}_{}.fc1/2.npy".format(in_planes, gen_planes))
        conv1_mat = np.load(fc1_save_name)
        conv2_mat = np.load(fc2_save_name)
...
class MlpCoder(nn.Module):
    """ MLPCoder as used in Vision Transformer, MLP-Mixer and related networks
    """
    def __init__(
            self,
            in_features,
        ...
        self.fc1 = linear_layer(self.in_features, self.hidden_features, bias=bias[0])
        self.act = act_layer()
        self.drop1 = nn.Dropout(drop_probs[0])
        self.fc2 = linear_layer(self.hidden_features, self.out_features, bias=bias[1])
        self.drop2 = nn.Dropout(drop_probs[1])
        # Add norm layers.
        self.norm_layer = norm_layer(self.out_features)
        # Calculate gen features.
        self.gen_features = self.in_features // 2

        self.fc1_mask = None
        self.fc2_mask = None

    # Init Weights for Coder.
    def init_weights(self):
        if self.in_features == self.out_features:
            print("In feats: {}, Gen Feats: {}".format(self.in_features, self.gen_features))
            # Load regular weight matrix
            fc1_normal_mat = kaiming_normal_matrix(self.in_features, self.hidden_features - self.gen_features)
            # [rep_3x3, squeezed, 3, 3]
            fc2_normal_mat = kaiming_normal_matrix(self.hidden_features - self.gen_features, self.out_features)
            k_path = "convcoder_export/mlp"
            fc1_path_mat, fc2_path_mat = load_matrix_pair(self.in_features, self.gen_features, k_path)
            fc1_full_mat = torch.cat([fc1_normal_mat, fc1_path_mat], dim=0)
            fc2_full_mat = torch.cat([fc2_normal_mat, fc2_path_mat], dim=1)
            # Assign weight values.
            self.fc1.weight.data = fc1_full_mat
            self.fc2.weight.data = fc2_full_mat
            # Relocate bias if any.
            if self.fc1.bias is not None:
                torch.nn.init.zeros_(self.fc1.bias)
            if self.fc2.bias is not None:
                torch.nn.init.zeros_(self.fc2.bias)

            # generate mask for convcoder.
            mask = torch.from_numpy(np.array([1.] * (self.hidden_features - self.gen_features) + [0.] * self.gen_features)).float()
            self.fc1_mask = torch.nn.parameter.Parameter(mask.view([-1, 1]), requires_grad=False)
            self.fc2_mask = torch.nn.parameter.Parameter(mask.view([1, -1]), requires_grad=False)
        else:
            torch.nn.init.kaiming_normal_(self.fc1.weight, a=math.sqrt(5.0))
            torch.nn.init.kaiming_normal_(self.fc2.weight, a=math.sqrt(5.0))
            if self.fc1.bias is not None:
                torch.nn.init.zeros_(self.fc1.bias)
            if self.fc2.bias is not None:
                torch.nn.init.zeros_(self.fc2.bias)
    ...
    def mask_arch_grads(self):
        if self.fc1_mask is not None and self.fc2_mask is not None:
            self.fc1.weight.grad.data = self.fc1.weight.grad.data * self.fc1_mask
            self.fc2.weight.grad.data = self.fc2.weight.grad.data * self.fc2_mask
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mlp = MlpCoder(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer, drop=drop)
...
class SwinTransformerCoder(nn.Module):
    r""" Swin Transformer: Coder version
        A PyTorch impl of : `Swin Transformer: Hierarchical Vision Transformer using Shifted Windows`  -
          https://arxiv.org/pdf/2103.14030
    ...
    def mask_arch_grads(self):
        for layer in self.layers:
            layer.mask_arch_grads()
    


def _create_swin_transformer_coder(variant, pretrained=False, **kwargs):
    model = build_model_with_cfg(
        SwinTransformerCoder, variant, pretrained,
...
@register_model
def swin_coder_s3_tiny_224(pretrained=False, **kwargs):
    ...
    return _create_swin_transformer_coder('swin_coder_s3_tiny_224', pretrained=pretrained, **model_kwargs)


@register_model
def swin_coder_s3_small_224(pretrained=False, **kwargs):
    ...
    model_kwargs = dict(
        ...
        num_heads=(3, 6, 12, 24), **kwargs)
    return _create_swin_transformer_coder('swin_coder_s3_small_224', pretrained=pretrained, **model_kwargs)


@register_model
def swin_coder_s3_base_224(pretrained=False, **kwargs):
    ...
    model_kwargs = dict(
        ...
        num_heads=(3, 6, 12, 24), **kwargs)
    return _create_swin_transformer_coder('swin_coder_s3_base_224', pretrained=pretrained, **model_kwargs)
