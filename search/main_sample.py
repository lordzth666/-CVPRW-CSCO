...
from src.metagraph_v3.metagraph import MetaGraph_v3  # noqa: E402
from src.proto.writer import ProtoWriter  # noqa: E402
from src.proto.contrib import *  # noqa: E402, F403
from src.estimator.imagenet_estimator import ImageNetEstimator  # noqa: E402
from src.estimator.cifar10_estimator import CIFAR10Estimator  # noqa: E402
from src.yaml_utils.yaml_parser import load_and_apply_yaml_config  # noqa: E402

from src.metagraph_v3.util_graph import random_connected_graph  # noqa: E402
...
def main(args):
    ...
    proto_dir = "temp/temp_search_{}_shard_{}".format(args.task, args.worker_id)
    ...
    if args.task in ["imagenet"]:
        ...
        raise NotImplementedError
    NUM_INTS = 100 if args.profile_only else 2
    i = 0
    while i < args.budget:
        ...
        writer.add(Convolutional_Proto(name="conv_stem",  # noqa: F405
                                      input="input",
                                      kernel_size=3,
        ...
        if args.task in ["imagenet"]:
            ...
            writer.add(DepthwiseConv_Proto(name="conv_stem_dw3x3",  # noqa: F405
                                      input="conv_stem",
                                      kernel_size=3,
                                      strides=1,
            ...
            # Add 1 depthwise layer for 1st stride=2 block on ImageNet.
            writer.add(Convolutional_Proto(name="conv_stem_1x1",  # noqa: F405
        ...
        if ps['num_head_conv'] != 0:
            writer.add(Convolutional_Proto(name='conv1x1',  # noqa: F405
        ...
        writer.add(GlobalAvgPool_proto(name="avg_1k",  # noqa: F405
                                      input=out_name))
        out_name = "avg_1k"
        ...
        metrics = estimator.train_and_evaluate(
            config, max_model_mac=args.max_mac_limit, profile_only=args.profile_only, validate_last_only=True)
        if metrics is not None:
            metrics.update({"hash": mask_hash})
            records.append(metrics)
            if i % NUM_INTS == 0 or i == args.budget - 1:
                print("Step {}".format(i))
