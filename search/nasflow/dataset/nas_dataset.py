    ...
    def __init__(
        ...
        if self.max_shards is not None and self.max_shards < len(self.all_directories):
            selected_indices = np.random.choice(
                np.arange(len(self.all_directories)), self.max_shards, replace=False)
            self.all_directories = self.all_directories[selected_indices]
        # Initialize train records and test records.
        self.train_records = None
        self.test_records = None
        self.max_records = max_records
        self.load_and_split_data(test_size=test_size, random_state=random_state)
        self.cache = cache
        if self.cache:
