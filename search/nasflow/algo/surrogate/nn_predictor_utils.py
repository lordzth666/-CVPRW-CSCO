...
def init_weights(m):
    if isinstance(m, nn.Linear):
        torch.nn.init.kaiming_normal_(m.weight.data, a=sqrt(5.0))
        if m.bias is not None:
            torch.nn.init.zeros_(m.bias.data)
    elif isinstance(m, nn.Embedding):
        torch.nn.init.kaiming_uniform_(m.weight.data, a=sqrt(5.0))
    else:
        pass
...
class NNBackBoneEmbedding(NNModelBase):
    def __init__(
        ...
        self.model_arch = nn.Sequential(*module_list)
        self.model_head = nn.Linear(all_units[-1], 1)
        # Add: 02/15/2023. Add a scaler to scale to different variations.
        self.scaler = nn.Parameter(
            torch.Tensor(np.array(1.0)), requires_grad=True)
        self.scaler_bias = nn.Parameter(
            torch.Tensor(np.array(0.0)), requires_grad=True)
    ...
    def forward(self, x):
        ...
        # Add scaler and bias.
        out = out * self.scaler + self.scaler_bias
        return out
    ...
    def __init__(
        ...
        if self.normalize_adj_mat:
            ...
            self.batchnorm_input = nn.BatchNorm1d(
                self.in_dims, eps=1e-5, momentum=0.1, affine=False)
        all_units = [self.in_dims] + self.units
        module_list = []
        for idx in range(len(all_units)-1):
            module_list.append(nn.Linear(all_units[idx], all_units[idx+1], bias=True))
            # module_list.append(nn.BatchNorm1d(all_units[idx+1], eps=1e-3, momentum=0.1))
            # module_list.append(nn.LayerNorm(all_units[idx+1], eps=1e-5))
            module_list.append(nn.ReLU(True))
        self.model_arch = nn.Sequential(*module_list)
        self.model_head = nn.Linear(all_units[-1], 1)
        # Add: 02/15/2023. Add a scaler to scale to different variations.
